...
## vector at the n observations, and n is the number of iterations for
## the monte carlo sampling.
function [chain, acceptanceRate] = mcmc_mhgibbs(x0_theta, X, y, batchlength, batches, p_tuning, p_acceptance = 0.23, aTheta = 3/500, bTheta = 1, nugget = 0, FUN = @(x) 1, verbose = 0, p=2)
  ...
  ## temporary accepted values
  pi_accept = q_accept = beta_accept = theta_accept = beta_old = sigma_old = beta_new = sigma_new = [];
  ...
  [q_old, beta_old, sigma_old] = krig_likelihood(logitinv(x.theta, aTheta, bTheta), X, y, F, nugget, p);
  ...
  for b = 0:(batches-1)
    for i = 1:batchlength
      ...
      for thetas = 1:dims
        ...
        ## step 2: calcuate the probability of move
	[q_new, beta_new, sigma_new] = krig_likelihood(logitinv(x.theta, aTheta, bTheta), X, y, F, nugget, p);
        phi_cand = logitinv(x.theta, aTheta, bTheta);
...
##      if rand() <= min(ratio, 1)
        ...
        if u <= exp(ratio)
          batchaccept(thetas)++;
          q_accept = q_new;
          beta_accept = beta_new;
          theta_accept = x.theta;
          sigma_accept = sigma_new;
          q_old = q_new;
          pi_old = pi_new;
          beta_old = beta_new;
          sigma_old = sigma_new;
...
function [chain, acceptanceRate] = mcmc_mhgibbsStochastic(x0_theta, X, y, C, x0_sigma, batchlength, batches, p_tuning, p_acceptance = 0.23, aTheta = 3/500, bTheta = 1, nugget = 0, FUN = @(x) 1, verbose = 0, p=2)
