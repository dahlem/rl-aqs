...
#                   function do no sort and the matlab function do...
# Version 1.2   (2009 March)
#    - correction : fixed minor problem in KS statistic calculation that would
#                   return incorrect value when xmin=1
#
...
#
   if( fdattype=="integer" ){
    ...
    for( xm in 1:length(xmins) ){
      ...
      # non-matrix version, suggested by Jeff Stuckman
      # (matrix version commented out, directly after)
       zdiff<-sapply(vec,function(a){sum((1:(xmin-1))^(-a))})
      ...
      I <- which.max(L)
      # compute KS statistic
      # (thanks to Jeff Stuckman for pointing out a small bug in the old version of this next line)
      fit <- cumsum(((xmin:xmax)^-vec[I])) / (zvec[I] - ifelse(xmin==1,0,sum((1:(xmin-1))^-vec[I])))
