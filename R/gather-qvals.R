...
df <- data.frame(qvalMean=rep(0, max(simulations$sim_num)),
                 qvalStdDev=rep(0, max(simulations$sim_num)));

## (1) for all simulations
for (sim in simulations$sim_num) {
  graphs <- simulations[simulations$sim_num == sim,]$graphs

  dfreps <- data.frame(qvals=rep(0, simulations[simulations$sim_num == sim,]$actual_reps));
  ...
  for (i in 1:(simulations[simulations$sim_num == sim,]$actual_reps)) {
    ...
    dfreps$qvals[i] <- sum(qval)
  }

  ## (5) take mean
  df$qvalMean[sim] <- mean(dfreps$qvals)
  df$qvalStdDev[sim] <- sd(dfreps$qvals)
}
  
## (6) write mean, std.dev. into "q-values.dat"
write.table(df, "q-values.dat", sep=",", row.names=FALSE, append=TRUE, quote=FALSE)
