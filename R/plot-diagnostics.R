...
des.diagnostic.plots <- function(cl, simNum, replications, graphs, stopTime, ps=TRUE) {
  ...
  if (ps) {
    des.pdf(paste(simNum, "/graph-response-diagnostic-evo-plot.pdf", sep=""))
  }
  ...
  if (ps) {
    des.pdf(paste(simNum, "/graph-delay-diagnostic-evo-plot.pdf", sep=""))
  }
  ...
  if (ps) {
    des.pdf(paste(simNum, "/graph-util-diagnostic-evo-plot.pdf", sep=""))
  }
...
if (FALSE) {
  ...
  clusterEvalQ(cl, library(desStats))
}
